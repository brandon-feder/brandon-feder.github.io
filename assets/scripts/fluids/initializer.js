...
const dither = "./dither.png";

let active = false;
let cancel = false;
let ditherURL = "./dither.png";
...
export function initWebGL(canvas) {
  ...
  function getFormats() {
    ...
    const HALF_FLOAT_TEXTURE_TYPE = isWebGL2
      ? webGL.HALF_FLOAT
...
export function activator(canvas, webGL, colorFormat, PROGRAMS, pointers) {
   ...
   */
  let ditheringTexture = PARAMS.embedded_dither
  ...
  function init() {
    ...
    const filtering = colorFormat.supportLinearFiltering
      ? webGL.LINEAR
      : webGL.NEAREST;
  ...
  function render(target) {
    ...
    if (PARAMS.render_shaders) {
      let program = PARAMS.render_bloom
